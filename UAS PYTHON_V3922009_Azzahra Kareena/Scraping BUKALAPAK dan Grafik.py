...
write = csv.writer(open('output/{}.csv'.format(key), 'w', newline=''))  # Membuka file CSV baru dengan nama file sesuai dengan kata kunci yang dimasukkan
...
for page in range(1, 3):  # Melakukan pengulangan untuk mengambil beberapa halaman data
    ...
    for p in products:  # Melakukan pengulangan untuk setiap produk
        ...
        write = csv.writer(open('output/{}.csv'.format(key), 'a', newline=''))  # Membuka file CSV dalam mode append
...
data = []
with open('output/baju.csv'.format(key), 'r') as file:
...
plt.savefig('output/line chart.jpg', format='jpg')
...
# Menyimpan diagram sebagai file JPG
plt.savefig('output/bar chart.jpg', format='jpg')
...
# Membaca data dari file CSV
data = []
with open('output/baju.csv', 'r') as file:
    reader = csv.reader(file)
    next(reader)  # Mengabaikan header
    for row in reader:
        data.append(row)
...
# Menyimpan diagram sebagai file JPG
plt.savefig('output/histogram.jpg', format='jpg')
...
# Menyimpan diagram sebagai file JPG
plt.savefig('output/pie chart.jpg', format='jpg')
...
# Membaca file CSV
data = pd.read_csv('output/{}.csv'.format(key))
...
# Menyimpan diagram sebagai file JPG
plt.savefig('output/scatter plot.jpg', format='jpg')
